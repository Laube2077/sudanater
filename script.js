document.addEventListener('DOMContentLoaded', () => {
    ...
    function goBackOneQuestion() {
        if (questionCount > 0 && questionHistory.length > 1) {
            questionHistory.pop(); // 現在の質問を履歴から削除
            const previousQuestion = questionHistory[questionHistory.length - 1]; // 1つ前の質問を再取得
            questionCount--;

            // UIを更新
            questionText.textContent = previousQuestion;
    ...
    answerButtons.addEventListener('click', (event) => {
        ...
        if (isRegression) {
            ...
            setTimeout(() => {
                goBackOneQuestion(); // 共通の「戻る」関数を呼び出す
